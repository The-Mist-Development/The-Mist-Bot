...
async function gamePriceSync() {
        ...
        for (let i = 0; i < responses.length; i++) {
            ...
            if (response.price_overview) {
                let oldPrice = await db.updateGame(gameid, response.price_overview.final)
                ...
                let embed = new EmbedBuilder()
                    ...
                    client.users.fetch(users[i]).then(user => {
                        user.send({ content: `**${response2.name}** is on sale on Steam!`, embeds: [embed] })
                            ...
                            .catch(err => handleDMError(err, users[i], failedUsers));
                    });
            }
            else {
...
async function handleDMError(err, userId, failedUsers) {
    ...
    if (!(errorString.includes("50007") || errorString.includes("Cannot send messages to this user"))) return;
